...
export const getAllEmployees = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(
      'http://localhost:4000/api/v1/employee/getAllEmployees',
      {
...
export const addEmployee = createAsyncThunk(
  ...
  async (employeeData) => {
    ...
    const response = await axios.post(
      'http://localhost:4000/api/v1/employee/addEmployee',
      employeeData
    )
    console.log(response.data)
    ...
    async ({ employeeId, payload }) => {
        try {
            const response = await axios.put(
                `http://localhost:4000/api/v1/employee/${employeeId}/with-relations`,
...
export const deleteEmployee = createAsyncThunk(
  ...
  async (employeeId) => {
    try {
      const response = await axios.post(
        'http://localhost:4000/api/v1/employee/deleteEmployee',
        { employeeId }
      )
...
export const moveEmployee = createAsyncThunk(
  ...
  async ({ employeeIds, toDepartmentId }, { dispatch, rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:4000/api/v1/employee/moveEmployee',
        {
          employeeIds,
          toDepartmentId,
        }
...
export const deleteMultipleEmployees = createAsyncThunk(
  ...
  async (employeeIds, { dispatch, rejectWithValue }) => {
    try {
      const response = await axios.post(
        'http://localhost:4000/api/v1/employee/deleteMultipleEmployees',
        employeeIds
      )
